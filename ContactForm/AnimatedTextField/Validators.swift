...
enum VaildatorFactory {
    static func validatorFor(type: ValidatorType) -> ValidatorConvertible {
        ...
        case .username: return UserNameValidator(NameType.userName)
        case .firstname: return UserNameValidator(NameType.firstName)
        case .lastname: return UserNameValidator(.lastName)
...
struct UserNameValidator: ValidatorConvertible {
    ...
    init(_ nameT: NameType) {
        nameType = nameT
    }
    func validated(_ value: String) throws -> String {
        guard value.count >= 3 else {
            throw ValidationError("\(nameType.errorName) must contain more than three characters" )
        }
        guard value.count < 18 else {
            throw ValidationError("\(nameType.errorName) shoudn't conain more than 18 characters" )
        }
        
        do {
            if try NSRegularExpression(pattern: "^[a-z]{1,18}$",  options: .caseInsensitive).firstMatch(in: value, options: [], range: NSRange(location: 0, length: value.count)) == nil {
                throw ValidationError("Invalid \(nameType.errorName), \(nameType.errorName) should not contain whitespaces, numbers or special characters")
            }
        } catch {
            throw ValidationError("Invalid \(nameType.errorName), \(nameType.errorName) should not contain whitespaces,  or special characters")
        }
        return value
    }
}
